...
public class MongoTemplateDecoratorImpl extends MongoTemplate implements MongoOperations, ApplicationContextAware, IndexOperationsProvider {
  ...
  public <T> MongoCollection<Document> createCollection(Class<T> entityClass) {
    return new MongoCollectionDecoratorImpl<>(getInvoker().invoke(() -> super.createCollection(entityClass)), getInvoker());
  }


  @Override
  public <T> MongoCollection<Document> createCollection(Class<T> entityClass, CollectionOptions collectionOptions) {
    return new MongoCollectionDecoratorImpl<>(getInvoker().invoke(() -> super.createCollection(entityClass, collectionOptions)), getInvoker());
  }

  @Override
  public MongoCollection<Document> createCollection(final String collectionName) {
    return new MongoCollectionDecoratorImpl<>(getInvoker().invoke(() -> super.createCollection(collectionName)), getInvoker());
  }

  @Override
  public MongoCollection<Document> createCollection(final String collectionName, final CollectionOptions collectionOptions) {
    return new MongoCollectionDecoratorImpl<>(getInvoker().invoke(() -> super.createCollection(collectionName, collectionOptions)), getInvoker());
  }

  @Override
  public MongoCollection<Document> getCollection(final String collectionName) {
    return new MongoCollectionDecoratorImpl<>(getInvoker().invoke(() -> super.getCollection(collectionName)), getInvoker());
  }
  ...
  public <O> CloseableIterator<O> aggregateStream(TypedAggregation<?> aggregation, String inputCollectionName, Class<O> outputType) {
    return new CloseableIteratorDecoratorImpl<>(getInvoker().invoke(() -> super.aggregateStream(aggregation, inputCollectionName, outputType)), getInvoker());
  }

  @Override
  public <O> CloseableIterator<O> aggregateStream(TypedAggregation<?> aggregation, Class<O> outputType) {
    return new CloseableIteratorDecoratorImpl<>(getInvoker().invoke(() -> super.aggregateStream(aggregation, outputType)), getInvoker());
  }

  @Override
  public <O> CloseableIterator<O> aggregateStream(Aggregation aggregation, Class<?> inputType, Class<O> outputType) {
    return new CloseableIteratorDecoratorImpl<>(getInvoker().invoke(() -> super.aggregateStream(aggregation, inputType, outputType)), getInvoker());
  }

  @Override
  public <O> CloseableIterator<O> aggregateStream(Aggregation aggregation, String collectionName, Class<O> outputType) {
    return new CloseableIteratorDecoratorImpl<>(getInvoker().invoke(() -> super.aggregateStream(aggregation, collectionName, outputType)), getInvoker());
  }
  ...
  public <T> ExecutableFind<T> query(Class<T> domainType) {
    return new ExecutableFindDecoratorImpl<>(getInvoker().invoke(() -> super.query(domainType)), getInvoker());
  }

  @Override
  public <T> ExecutableUpdate<T> update(Class<T> domainType) {
    return new ExecutableUpdateDecoratorImpl<>(getInvoker().invoke(() -> super.update(domainType)), getInvoker());
  }

  @Override
  public <T> ExecutableRemove<T> remove(Class<T> domainType) {
    return new ExecutableRemoveDecoratorImpl<>(getInvoker().invoke(() -> super.remove(domainType)), getInvoker());
  }

  @Override
  public <T> ExecutableAggregation<T> aggregateAndReturn(Class<T> domainType) {
    return new ExecutableAggregationDecoratorImpl<>(getInvoker().invoke(() -> super.aggregateAndReturn(domainType)), getInvoker());
  }

  @Override
  public <T> ExecutableInsert<T> insert(Class<T> domainType) {
    return new ExecutableInsertDecoratorImpl<>(getInvoker().invoke(() -> super.insert(domainType)), getInvoker());
  }
  ...
  public MongoDatabase getDb() {
    return new MongoDataBaseDecoratorImpl(getInvoker().invoke(super::getDb), getInvoker());
  }
  ...
  public <T> List<T> findDistinct(Query query, String field, Class<?> entityClass, Class<T> resultClass) {
    return getInvoker().invoke(() -> super.findDistinct(query, field, entityClass, resultClass));
  }

  @Override
  public <T> List<T> findDistinct(Query query, String field, String collectionName, Class<?> entityClass, Class<T> resultClass) {
    return getInvoker().invoke(() -> super.findDistinct(query, field, collectionName, entityClass, resultClass));
  }
  ...
  public <S, T> T findAndReplace(Query query, S replacement, FindAndReplaceOptions options, Class<S> entityType, String collectionName, Class<T> resultType) {
    return getInvoker().invoke(() -> super.findAndReplace(query, replacement, options, entityType, collectionName, resultType));
  }

  @Override
  public <T> ExecutableMapReduce<T> mapReduce(Class<T> domainType) {
    return getInvoker().invoke(() -> new ExecutableMapReduceDecoratorImpl<>(super.mapReduce(domainType), getInvoker()));
  }

  @Override
  public MongoTemplateDecoratorImpl withSession(ClientSession session) {
    return getInvoker().invoke(() -> new MongoTemplateDecoratorImpl(super.withSession(session), getInvoker()));
  }

  @Override
  public IndexOperations indexOps(String collectionName) {
    return getInvoker().invoke(() -> new IndexOperationsDecoratorImpl(super.indexOps(collectionName), getInvoker()));
  }

  @Override
  public IndexOperations indexOps(Class<?> entityClass) {
    return getInvoker().invoke(() -> new IndexOperationsDecoratorImpl(super.indexOps(entityClass), getInvoker()));
  }

  @Override
  public BulkOperations bulkOps(BulkOperations.BulkMode mode, String collectionName) {
    return new BulkOperationsDecoratorImpl(getInvoker().invoke(() -> super.bulkOps(mode, collectionName)), getInvoker());
  }

  @Override
  public BulkOperations bulkOps(BulkOperations.BulkMode mode, Class<?> entityType) {
    return new BulkOperationsDecoratorImpl(getInvoker().invoke(() -> super.bulkOps(mode, entityType)), getInvoker());
  }

  @Override
  public BulkOperations bulkOps(BulkOperations.BulkMode mode, Class<?> entityType, String collectionName) {
    return new BulkOperationsDecoratorImpl(getInvoker().invoke(() -> super.bulkOps(mode, entityType, collectionName)), getInvoker());
  }

  @Override
  public ScriptOperations scriptOps() {
    return new ScriptOperationsDecoratorImpl(getInvoker().invoke(super::scriptOps), getInvoker());
  }

  @Override
  public SessionScoped withSession(ClientSessionOptions sessionOptions) {
    return getInvoker().invoke(() -> new SessionScopedDecoratorImpl(super.withSession(sessionOptions), getInvoker()));
  }

  @Override
  public SessionScoped withSession(Supplier<ClientSession> sessionProvider) {
    return getInvoker().invoke(() -> new SessionScopedDecoratorImpl(super.withSession(sessionProvider), getInvoker()));
  }

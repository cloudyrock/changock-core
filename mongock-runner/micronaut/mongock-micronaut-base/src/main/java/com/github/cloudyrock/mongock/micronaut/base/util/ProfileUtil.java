package com.github.cloudyrock.mongock.micronaut.base.util;
...
public final class ProfileUtil {
  ...
  public static boolean matchesActiveEnvironmentProfile(List<String> activeProfiles,
                                                        Class<? extends Annotation> annotation,
                                                        AnnotatedElement element,
                                                        Function<AnnotatedElement, String[]> profileExtractor) {
    if (!element.isAnnotationPresent(annotation)) {
      return true; // no-profiled changeset always matches
    }
    boolean containsActiveProfile = false;
    for (String profile : profileExtractor.apply(element)) {
